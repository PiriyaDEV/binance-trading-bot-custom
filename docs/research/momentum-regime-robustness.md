...
## `rideMode`: widen the trail through a confirmed strong trend — same trade-off, still not a fix

A lighter-weight version of "next direction" #5 below, tried first because it reuses `regimeFilter`'s existing BTC-anchored gate rather than building a whole second strategy. Mechanism (`packages/strategy/momentum/src/tick.ts`'s `rideModeActive()`, new `rideMode` config block in `schema.ts`, off by default): while `regimeFilter` confirms direction AND the reference market's own ADX (Wilder's trend-strength indicator, newly exported from `packages/indicators`) reads above `adxThreshold` — not just "on the right side of the line," but _strongly, persistently_ trending — a matching open position ignores the ordinary EMA-cross exit and trails at a wider `retracePct` instead of `trailingStopPct`. The moment the regime or its strength breaks, normal exit behaviour resumes on the very next tick; nothing is persisted. Applies symmetrically to both legs of `direction:'both'`: a long rides a confirmed strong uptrend, a short rides a confirmed strong downtrend.

Coarse sweep on `adxThreshold` (`retracePct` fixed at 0.20, `adxPeriod` at Wilder's default 14), layered on top of the adopted `regimeFilter period=30` baseline, same bear/bull windows as every other experiment above:

| adxThreshold | Bear return / alpha / PF | Bear OOS (alpha/PF/trades) | Bull return / alpha / PF | Bull OOS (alpha/PF/trades) |
| --- | --- | --- | --- | --- |
| _no rideMode (baseline)_ | +26.6% / +82.4% / 4.55 | +0.9% / 0.50 / 18 | +27.7% / −42.7% / 1.64 | −6.6% / 1.20 / 32 |
| 25 | +21.9% / +77.8% / 3.59 | +0.2% / 0.74 / **14** | **+36.0% / −34.4% / 2.23** | −5.4% / **2.64** / **38** |
| 30 | +24.6% / +80.4% / 3.92 | **−0.9% / 0.38 / 15** | +35.5% / −34.8% / 2.14 | −7.4% / **3.07** / 31 |
| 35 | **+28.1% / +84.0% / 4.69** | +0.5% / 0.81 / 15 | +29.4% / −41.0% / 1.81 | −5.9% / 2.36 / 32 |
| 40 | +26.5% / +82.3% / 4.52 | +0.7% / 0.36 / 17 | +27.1% / −43.3% / 1.65 | −6.5% / 1.46 / 30 |
| 50 | _identical to baseline — never engages at this threshold on this basket_ |  |  |  |

**The exact same structural trade-off as every prior attempt, just via a different mechanism.** `adxThreshold` 25–30 is where the bull-year number actually moves — alpha from −42.7% to about −34.5%, a real ~8 percentage-point close, with better absolute return, profit factor, and OOS metrics across the board in that window. But the SAME low thresholds cost the bear year: alpha down 2–5pp, profit factor down roughly 1 full point, and — worse — the bear-year OOS trade count, already marginal at 18 against a 20 minimum, drops further to 14–15. At `adxThreshold=30` specifically, bear-year **OOS alpha flips from a passing +0.9% to a failing −0.9%** — riding a short through what regime data called a strong downtrend held the position into what should have counted as an out-of-sample loss, turning a criterion that already passed into a new failure, not just widening an existing one. `adxThreshold=35` is the one variant that improves BOTH windows' headline numbers (bear alpha +84.0% vs +82.4%, bull alpha −41.0% vs −42.7%) without flipping any gate criterion — but it still pushes bear-year OOS trades from 18 to 15, moving the wrong direction on the one metric already failing, so it isn't a clean win either.

**Verdict: not adopted.** `rideMode` stays `enabled:false` by default (no behaviour change for any existing profile). No tested threshold clears the bar its own plan set — "helps bull-year alpha without breaking bear-year gate criteria further" — so per that plan, no follow-up `retracePct` sweep is warranted on this mechanism; the ceiling looks structural, not a tuning gap. This confirms (with a genuinely different lever — widening exits on a strength signal, rather than gating entries on a direction signal) the same conclusion the trend-gate and `regimeFilter` knob searches already reached: closing the bull-year gap by changing how long a position survives, on either side of the trade, keeps trading away bear-year robustness. The next real lever is a structurally different one — see directions 4–5 below.

## Recommended next directions

1. ~~Market-wide regime signal, not per-symbol~~ — **done.** ~~Fine-tune the period~~ — **done, period=30.** ~~Search `regimeFilter`'s remaining knobs~~ — **done, none beat period=30 (see above).** ~~Widen exits through a confirmed strong trend (`rideMode`)~~ — **done, same trade-off, not adopted (see above).** Every entry-time and exit-time lever on this single fast-EMA-cross strategy has now been tried; further gains need a structurally different architecture, not another knob on this one.
2. **Fix the bear-year OOS trade-count shortfall** at period=30 (18 vs the 20 minimum) — confirmed NOT fixable by tuning `regimeFilter` itself (see the knob search above), and actively WORSENED by every `rideMode` variant tried (14–17 vs the baseline's 18). Needs a different lever: a longer bear-year test window (tried once above, traded this failure for an OOS-alpha failure instead — not a clean fix either), or a policy-level exception.
...
4. **Accept the remaining gap, add a portfolio allocator.** period=30 narrows but doesn't close the bull-year underperformance; a size-reducing or pausing allocator layered on top (rather than a pure entry-time gate, and rather than `rideMode`'s pure exit-time widening) could close more of the remaining gap without another architecture change.
5. **A second, different strategy for strong bull markets**, switched by a regime classifier — still viable if 2–4 plateau, but no longer the only path forward now that (1) has shown real, measurable progress. `rideMode`'s result suggests this needs to be a genuinely different position-management style (e.g. closer to buy-and-hold, not just a wider trail on the same EMA-cross strategy) to actually close the gap rather than just shrink it at the bear year's expense.
